...
export class BookingService {
    ...
    private buildWhereCondition(userId: number, query: BookingQueryDto): Prisma.BookingWhereInput {
        ...
        if (query.dateFrom || query.dateTo) {
            const dateField = query.dateType === 'checkIn' ? 'checkInDate' : 'checkOutDate';
    ...
    private buildOrderBy(query: BookingQueryDto): Prisma.BookingOrderByWithRelationInput {
        const orderBy: Prisma.BookingOrderByWithRelationInput = {};
        if (query.orderBy) {
            orderBy[query.orderBy] = query.order || 'desc';
        } else {
            orderBy[query.dateType === 'checkIn' ? 'checkInDate' : 'checkOutDate'] = query.order || 'desc';
        }
        return orderBy;
    }
